...
public class Enemy {
	   ...
	   public float getNextScoutX(){
		   if (attackDirection == Engine.ATTACK_LEFT){
			   return (float)((Engine.BEZIER_X_4*(posT*posT*posT)) + (Engine.BEZIER_X_3 * 3 * (posT * posT) * (1-posT)) + (Engine.BEZIER_X_2 * 3 * posT * ((1-posT) * (1-posT))) + (Engine.BEZIER_X_1 * ((1-posT) * (1-posT) * (1-posT))));
		   }else{
			   return (float)((Engine.BEZIER_X_1*(posT*posT*posT)) + (Engine.BEZIER_X_2 * 3 * (posT * posT) * (1-posT)) + (Engine.BEZIER_X_3 * 3 * posT * ((1-posT) * (1-posT))) + (Engine.BEZIER_X_4 * ((1-posT) * (1-posT) * (1-posT))));
		   }
		   
	   }
	   public float getNextScoutY(){
		   return (float)((Engine.BEZIER_Y_1*(posT*posT*posT)) + (Engine.BEZIER_Y_2 * 3 * (posT * posT) * (1-posT)) + (Engine.BEZIER_Y_3 * 3 * posT * ((1-posT) * (1-posT))) + (Engine.BEZIER_Y_4 * ((1-posT) * (1-posT) * (1-posT))));
	   }
